    ...
    def boil(self):
        ...
        if self.__water_level == 0:  # Поверка уровня воды
            ...
            if temperature >= self.__temperature_stop:  # Если достигнута нужная температура
                ...
                posts.append({'body': 'Достигнута необходимая температура'})
                print('Достигнута необходимая температура')
            logging.info('Чайник закипел')
            posts.append({'body': 'Чайник закипел'})
    ...
    def input_with_timeout(self, timeout, timer=time.monotonic):
        ...
        while timer() < endtime:
            ...
            if msvcrt.kbhit():  # Если пользователь нажал клавишу, выключить чайник
                ...
                posts.append({'body': 'Чайник остановлен'})
...
def index():
    """Запуск функции для работы с кнопками включения и выключения чайника через сайт."""
    global count
    if count:
        if request.method == 'POST':  # Обработка методов POST
            if request.form.get('action1') == 'START':
                k.button_start()
            elif request.form.get('action2') == 'STOP':
                k.button_stop()
        elif request.method == 'GET':  # Обработка методов GET
            return render_template("index.html", posts=posts)
    else:
        count = True
    return render_template("index.html", posts=posts)  # Вывод страницы
